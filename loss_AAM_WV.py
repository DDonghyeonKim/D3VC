...
def RKDA(student, teacher):
    ...
    loss = torch.sum(torch.abs(s_angle-t_angle))
    return loss

def compute_d_loss(nets,WV, args, x_real, y_org, y_trg, z_trg=None, x_ref=None, use_r1_reg=True, use_adv_cls=False, use_con_reg=False):
    args = Munch(args)
    ...
    with torch.no_grad():
        ...
        x_fake = nets.generator(x_real, s_trg, masks=None, F0=None)
    ...
    if use_adv_cls:
        out_de,prob = nets.discriminator.classifier(x_fake,y_org)
        loss_real_adv_cls = F.cross_entropy(prob[y_org != y_trg], y_org[y_org != y_trg])

        if use_con_reg:
            out_de_aug,_ = nets.discriminator.classifier(t(x_fake).detach(),y_org)
    ...
    loss = loss_real + loss_fake + args.lambda_reg * loss_reg + \
            ...
            args.lambda_con_reg * loss_con_reg + reg_loss*0.1
...
def compute_g_loss(nets,WV, args, x_real, y_org, y_trg, z_trgs=None, x_refs=None, use_adv_cls=False):
    args = Munch(args)
    ...
    # adversarial loss
    x_fake = nets.generator(x_real, s_trg, masks=None, F0=None)
    C_emb=nets.generator.Middle(x_real)
    sdtw = SoftDTW(use_cuda=True, gamma=0.1)
#    print(C_emb.shape)##189
#    print(WV.shape)##119
    dtw_loss = (sdtw(torch.permute(C_emb,[0,2,1]), torch.permute(WV.detach(),[0,2,1]))).mean()
    out = nets.discriminator(x_fake, y_trg)
...
#    loss_f0 = f0_loss(F0_fake, F0_real)
    ...
    x_fake2 = nets.generator(x_real, s_trg2, masks=None, F0=None)
...
#    loss_ds += F.smooth_l1_loss(GAN_F0_fake, GAN_F0_fake2.detach())
    ...
    #s_aug = nets.style_encoder(torch.flip(x_real,[3]), y_org)

    #Z_org=(s_org-torch.mean(s_org,0,keepdim=True))/(torch.std(s_org,0,keepdim=True)+0.0001)
    #Z_aug=(s_org-torch.mean(s_aug,0,keepdim=True))/(torch.std(s_aug,0,keepdim=True)+0.0001)

    #C=torch.matmul(torch.permute(Z_org,[1,0]),Z_aug)/20

    #c_diff=torch.pow(C-torch.eye(192).cuda(),2)

    #off_matrix=torch.ones((192,192)).cuda()-torch.eye(192).cuda()

    #Bar_loss=torch.sum(((off_matrix*c_diff)*0.0005 + torch.eye(192).cuda())*c_diff)

    x_rec = nets.generator(x_fake, s_org, masks=None, F0=None)


    loss_cyc = torch.mean(torch.abs(x_rec - x_real))
    #Real_middle=nets.generator.Middle(x_real, s_trg, masks=None, F0=GAN_F0_real)
    #Fake_middle = nets.generator.Middle(x_fake, s_org, masks=None, F0=GAN_F0_fake)
    #Middle_embedding=F.smooth_l1_loss(Fake_middle, Real_middle)
    ...
    # adversarial classifier loss
    if use_adv_cls:
        _,prob = nets.discriminator.classifier(x_fake,y_trg)

        loss_adv_cls = F.cross_entropy(prob[y_org != y_trg], y_trg[y_org != y_trg])
    else:
    ...
    loss = args.lambda_adv * loss_adv + args.lambda_sty * loss_sty \
           ...
           + args.lambda_adv_cls * loss_adv_cls + dtw_loss
