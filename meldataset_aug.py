...
class MelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mel_tensor, label,raw_wav = self._load_data(data)
        ...
        return mel_tensor, label, ref_mel_tensor, ref2_mel_tensor, ref_label,raw_wav

    def _load_data(self, path,Aug=False):
        wave_tensor, label,wav_len = self._load_tensor(path)
        
        if not self.validation: # random scale for robustness
            random_scale = 0.5 + 0.5 * np.random.random()
            wave_tensor = random_scale * wave_tensor
            #if wav_len > 24000*2.39:
           #     random_start = np.random.randint(0, wav_len - int(24000*2.39))
            #    wave_tensor = wave_tensor[random_start:random_start + int(24000*2.39)]
        ...
        return mel_tensor, label,wave_tensor
    ...
    def _load_tensor(self, data):
        ...
        return wave_tensor, label,len(wave)
...
class Collater(object):
    ...
    def __call__(self, batch):
        ...
        Raw_audios = torch.zeros((batch_size, 57360)).float()
        labels = torch.zeros((batch_size)).long()
        ref_mels = torch.zeros((batch_size, nmels, self.max_mel_length)).float()
        ref2_mels = torch.zeros((batch_size, nmels, self.max_mel_length)).float()
        ref_labels = torch.zeros((batch_size)).long()

        for bid, (mel, label, ref_mel, ref2_mel, ref_label,raw_audio) in enumerate(batch):
            mel_size = mel.size(1)
            audio_size = raw_audio.size(0)
            mels[bid, :, :mel_size] = mel
            Raw_audios[bid,:audio_size]=raw_audio
        ...
        return mels, labels, ref_mels, ref2_mels, ref_labels, z_trg, z_trg2,Raw_audios
